...
if __name__ == '__main__':
    ...
    # Execute training configurations and record accuracy for each
    configs = [
        {'batch_size': args.batch_size, 'lr': args.lr, 'epochs': args.epochs},
        {'batch_size': 8, 'lr': args.lr, 'epochs': args.epochs},
        {'batch_size': 16, 'lr': args.lr, 'epochs': args.epochs},
        {'batch_size': args.batch_size, 'lr': 0.05, 'epochs': args.epochs},
        {'batch_size': args.batch_size, 'lr': 0.01, 'epochs': args.epochs},
        {'batch_size': args.batch_size, 'lr': args.lr, 'epochs': 20},
        {'batch_size': args.batch_size, 'lr': args.lr, 'epochs': 5}
    ]

    results = []
    for config in configs:
        print(f"Training with config: batch_size={config['batch_size']}, lr={config['lr']}, epochs={config['epochs']}")
        args.batch_size, args.lr, args.epochs = config['batch_size'], config['lr'], config['epochs']

        # Run main training function here
        acc = main(args)
        results.append(f"{acc:.2f}")
